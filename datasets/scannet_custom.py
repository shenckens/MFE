# Code taken from NeuralRecon (https://github.com/zju3dv/NeuralRecon)
# and adapted to fit custom dataloader.
...
class ScanNetWithNoisyDepth(Dataset):
    ...
    def read_cam_file(self, filepath, vid):
        ...
        extrinsics = np.loadtxt(os.path.join(
            filepath, 'pose', '{}.txt'.format(str(vid))))
    ...
    def __getitem__(self, idx):
        ...
        tsdf_list = self.read_scene_volumes(os.path.join(
            self.datapath, self.tsdf_file), meta['scene'])
